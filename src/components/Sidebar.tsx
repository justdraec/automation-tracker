  ...
  return (
    <div className="w-[256px] shrink-0 bg-surface-container-low border-r border-outline-variant/10 flex flex-col h-full">
      ...
      <div className="px-4 pt-5 pb-3">
        <div className="flex items-center gap-2.5 mb-5">
          <div className="w-9 h-9 rounded-2xl bg-primary flex items-center justify-center flex-shrink-0 shadow-lg shadow-primary/20">
            <svg width="18" height="18" viewBox="0 0 16 16" fill="none"><path d="M9 2L4 9h5l-2 5 7-8H9l2-5z" fill="white"/></svg>
          </div>
          <div>
      ...
      <div className="px-3 mb-2">
        <NavItem icon="smart_toy" label="AI Discovery" active={activeView === 'chat'} onClick={onNewOpportunity} />
        <NavItem icon="list_alt" label="Opportunities" active={activeView === 'list'} onClick={onSelectOpportunity} />
        {builderMode && (
          <NavItem icon="bar_chart" label="Build List" active={activeView === 'buildlist' || activeView === 'build-detail'} onClick={onOpenBuildList} />
        )}
        {builderMode && <NavItem icon="analytics" label="Analytics" disabled />}
        {builderMode && <NavItem icon="hub" label="Automation" disabled />}
      </div>

      {/* Recent opportunities */}
      <div className="flex-1 overflow-y-auto px-3 pb-2">
        <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/50 px-1 mb-2 mt-1">Recent</p>
        {entries.length === 0 ? (
          <p className="text-xs text-on-surface-variant/40 px-1 py-4 leading-relaxed">No opportunities yet. Start a discovery to capture your first one.</p>
        ) : (
          entries.slice(0, 5).map(e => (
            <button key={e.id} onClick={onSelectOpportunity}
  ...
  return (
    <button onClick={disabled ? undefined : onClick} disabled={disabled}
      className={`w-full flex items-center gap-3 px-3 py-2.5 mb-0.5 text-sm font-medium rounded-xl transition-all
        ${active ? 'border-l-4 border-primary bg-gradient-to-r from-primary-container/10 to-transparent text-primary font-semibold'
          : disabled ? 'text-on-surface/30 cursor-not-allowed'
  ...
  return (
    <button onClick={onClick} title={title}
      className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all
        ${active ? 'bg-primary/10 border border-primary/30 text-primary' : 'text-on-surface-variant hover:text-on-surface hover:bg-surface-container border border-transparent'}`}>
      {children}
    </button>
  )
}
